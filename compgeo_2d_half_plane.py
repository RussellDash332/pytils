...
if __name__ == '__main__':
    ...
    p = [(2, 0), (6, 0), (8, 3), (3, 5), (0, 2)]
    # CCW, so +1 instead of -1
    print(half_plane_intersection([(p[i], p[(i+1)%len(p)]) for i in range(len(p))]))

    # https://open.kattis.com/problems/bigbrother
    p = [(140, 62), (97, 141), (68, 156), (129, 145), (153, 176), (130, 109)]
    # CW, so -1 instead of +1
    print(half_plane_intersection([(p[i], p[(i-1)%len(p)]) for i in range(len(p))]))