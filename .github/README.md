...
- [`fenwick_tree_rsq`](https://github.com/RussellDash332/pytils/blob/main/fenwick_tree_rsq.py)
    ...
    Classic Fenwick tree to answer range sum queries (RSQ). All PURQ (point update, range query), RUPQ (range update, point query), and RURQ (range update, range query) versions are available. Updates and queries all run in $O(\log n)$ time, except for that of 2D Fenwick tree which runs in $O(\log^2 n)$ time.
