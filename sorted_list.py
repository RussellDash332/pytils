# quick copy for no custom function
from bisect import *
class SortedList:
    def __init__(s, l1=512, l2=128, mv=~(1<<30)):
...
from bisect import *
class SortedList:
    def __init__(s, A=None, key=None, l1=512, l2=128, mv=~(1<<30)):
        s.l1 = l1; s.l2 = l2; s.x1 = s.l1*2; s.x2 = s.l2*2; s.g1 = len(bin(s.x1))-3; s.g2 = len(bin(s.x2))-3; s.mv = mv; s.key = key; s.clear()
        if not A: return
        s.cnt = len(A:=sorted(A, key=key) if key else sorted(A))
        for l in range(0, s.cnt, s.l1): s.b.append(A[l:min(l+s.l1, s.cnt)])
        s._ex()
    def _k(s, x): return s.key(x) if s.key else x
    def clear(s): s.b = [[]]; s.bc = [0]; s.bm = [0]; s.sm = []; s.ss = []; s.cnt = 0
    def __len__(s): return s.cnt
    def _ex(s):
        bo = [*s.b]; c = sum(map(bool, bo)); sn = (c+s.l2-1)//s.l2; ec = s.cnt; s.clear(); s.cnt = ec; s.sm.extend([s.mv]*sn); s.ss.extend([0]*sn); i = 0
    ...
    def _lb(s, x):
        if not s.ss: return (0, len(s.b[0]))
        kx = s._k(x); l = -1; r = len(s.ss)-1
        while r-l > 1:
            if s._k(s.sm[m:=(l+r)>>1]) >= kx: r = m
            else: l = m
        l = r<<s.g2; r = r<<s.g2|s.ss[r]
        while r-l > 1:
            if s._k(s.bm[m:=(l+r)>>1]) >= kx: r = m
            else: l = m
        return (r, bisect_left(s.b[r], kx, key=s.key) if s.key else bisect_left(s.b[r], x))
    def _ub(s, x):
        if not s.ss: return (0, len(s.b[0]))
        kx = s._k(x); l = -1; r = len(s.ss)-1
        while r-l > 1:
        ...
        return (r, bisect(s.b[r], kx, key=s.key) if s.key else bisect(s.b[r], x))
    def _rbm(s, b1, b2):
        for i in range(b1, b2+1): s.bc[i] = 0
        for i in range(b1, b2+1):
            if s.b[i]: s.bc[i] += len(s.b[i]); s.bm[i] = s.b[i][-1]
    ...
    def append(s, x):
        ...
        bi, it = s._lb(x); i = bi
        while i < len(s.bc): s.bc[i] += 1; i += i&-i
        s.b[bi].insert(it, x); si = (bi-1)>>s.g2; s.bm[bi] = max(s.bm[bi], x, key=s._k); s.sm[si] = max(s.sm[si], x, key=s._k)
        if len(s.b[bi]) >= s.x1:
            bj = (si<<s.g2)|s.ss[si]; bn = si+1<<s.g2
            if len(s.b) <= bn: s.b.insert(bi+1, s.b[bi][s.l1:]); s.bc.append(0); s.bm.append(0)
            else: s.b[bi+2:bj+2] = s.b[bi+1:bj+1]; s.b[bi+1] = s.b[bi][s.l1:]
    ...
    def remove(s, x):
        kx = s._k(x)
        if not s.sm or s._k(s.sm[-1]) < kx: return
        bi, it = s._lb(x)
        if it == len(s.b[bi]) or s._k(s.b[bi][it]) > kx: return
        s.cnt -= 1; i = bi
        while i < len(s.bc): s.bc[i] -= 1; i += i&-i
        s.b[bi].pop(it); si = (bi-1)>>s.g2; bj = (si<<s.g2)|s.ss[si]
        if s.b[bi]:
...
if __name__ == '__main__':
    ...
    def verify(T, A, key=None):
        assert len(T) == len(A) and [T[i] for i in range(len(A))] == sorted(A, key=key)

    # actually works with dynamic comparators too as long as the relative order is maintained
    A = [3, 7, 2, 5, 8, 7]
    T = SortedList(A, key=lambda x: -x)
    verify(T, A, key=lambda x: -x)

    # how about non-integer entries? we can map accordingly
    A = ['hello', 'banana', 'banana', 'boy', 'pixie', 'guitar', 'zyzzy', 'a', '']
    S = [*range(len(A))]
    T = SortedList(S, key=lambda x: A[x])
    verify(T, S, key=lambda x: A[x])
